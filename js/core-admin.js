...
$(document).ready(function () {
   ...
   $(".tableadd").click(function (){
    ...
    $(".user-logins").click(function () {
        ...
        window.location = root+"/table/WATCHDOG?VALUE="+username;
    });
    ...
    $(".edit-user").click(function () {
        ...
        var element = controlElement.parents("tr").find("td")[2];
        var username = $(element).html();
        window.location = root+"/user/"+username;
    });
    ...
    $(document).on("click",".dbl_click_fk", function () {
        ...
        window.location = root +"/table/"+activetable+"/"+columnname+"/"+fk_val;
    });
    ...
    $(document).on("click",".rowdelete",function (){
     ...
     $(document).on("click",".rowbrowse",function(){
         ...
         window.location = root + "/insert/"+get_active_table()+"/"+id;
     });
});

function alter(table_name, fields, form_build_id){
    $.ajax({
            url: root + "/ajax/alter_table",
            type: 'POST',
            dataType: 'json',
...
function define(table_name, fields, form_build_id){
    $.ajax({
            url: root + "/ajax/new_table_definition",
            type: 'POST',
            dataType: 'json',
